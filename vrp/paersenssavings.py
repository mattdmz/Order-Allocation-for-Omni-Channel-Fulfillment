...
""" MODIFICATIONS: 
    ...
    This original file is VeRyPy.classical_heuristics.parallel_savings.py 
    (https://github.com/yorak/VeRyPy/blob/0cb7a83279352ecba83117357589c83e576edaa2/classic_heuristics/paessens_savings.py)

    Modifications: 
    - paessens_savings_init(parmeter added --> return_objv:None)
      return_objv: returns the solution as well as the obj_function's value if set to True
    
    - added import: from parameters import PS_88_MAX_ITER_LOC_SEARCH

    - addidtional parameter define othewhise set to None --> 
      do_local_search(max_iterations=max_iterations_ls)
      max_iterations_ls: PS_88_MAX_ITER_LOC_SEARCH (default)
...
from parameters import PS_88_MAX_ITER_LOC_SEARCH
...
def paessens_savings_function(D, g_multiplier, f_multiplier):
    ...
    for i in range(1,N):
        for j in range(i+1,N):
            s = D[i,0]+D[0,j]-g_multiplier*D[i,j]+f_multiplier*abs(D[i,0]-D[0,j])
...
def paessens_savings_init(D,d,C,L, minimize_K=False,
                          strategy="M4", do_3opt=True, max_iterations_ls=PS_88_MAX_ITER_LOC_SEARCH, return_objv=None):
    """
    ...
    * do_3opt (default True) optimize the resulting routes to 3-optimality
        ...
        try:
            ...
            if do_3opt:
                sol = do_local_search([do_3opt_move], sol, D, d, C, L,
                                      LSOPT.BEST_ACCEPT, max_iterations=max_iterations_ls)
    ...
    #added option to return obj value directly
    if return_objv == None:
        return best_sol
    else:                           
        return best_sol, objf(best_sol, D)